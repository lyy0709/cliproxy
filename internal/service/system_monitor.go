...
type APIKeyStats struct {
	Total    int64 `json:"total"`     // 总数
	Active   int64 `json:"active"`    // 活跃的（状态为 active）
	Disabled int64 `json:"disabled"`  // 已禁用
...
func (s *SystemMonitorService) GetAPIKeyStats() APIKeyStats {
	stats := APIKeyStats{}

	// 总数
	s.db.Model(&model.APIKey{}).Count(&stats.Total)

	// 活跃的（状态为 active）
	s.db.Model(&model.APIKey{}).
		Where("status = ?", "active").
		Count(&stats.Active)
