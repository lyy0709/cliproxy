...
// @Summary 获取系统监控数据
// @Description 获取系统资源、缓存、MySQL、账号、API Key、今日使用等完整监控数据
// @Tags 系统监控
// @Produce json
...
// @Description 获取 API Key 总数、活跃数量
// @Tags 系统监控
// @Produce json
// @Success 200 {object} response.Response{data=service.APIKeyStats}
// @Router /api/admin/monitor/api-keys [get]
func (h *SystemMonitorHandler) GetAPIKeyStats(c *gin.Context) {
	stats := h.monitorService.GetAPIKeyStats()
	response.Success(c, stats)
}
